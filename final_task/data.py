...
def check_add_info(url):
    ...
    data = {"host_information": speca['host_information'], "network": speca['network'], "disk": speca['disk'], "memory": speca['memory'], "cpu": speca['cpu'], "load_average": speca['load_average'], "battery_charge_percentage": speca['battery_charge_percentage'], "AC_power": f"Is power connected - {speca['AC_power']}"}
    if len(k) == 0:
    ...
    for el in k:
        ...
        if el['host_information']['hostname'] == name:
            pk = el['id']
            requests.put(f'{url}{pk}', json=data)
        ...
        if el['host_information']['hostname'] != name and name not in list_hostname:
            requests.post('{url}/add', json=data)
            logging.info(f'Received data about this device --> {speca}')
